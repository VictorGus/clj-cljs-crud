...
(defn patient-search [ctx]
  (fn [{{:keys [search-query]} :params :as req}]
...
(defn get-patient-by-identifier [ctx]
  (fn [{{:keys [iden]} :params :as req}]
    {:status 200
     :body {:entry (db/query-first {:select [:*]
                                    :from [:patient]
                                    :where (hsql/raw (str "resource->>'identifier' = " (str "'" iden "'")))} ctx)}}))
